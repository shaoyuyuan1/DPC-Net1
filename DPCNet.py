...
class Network:
    ...
    def building_block(self, xyz, feature, neigh_idx, d_out, name, is_training):
        ...
        local_rep, g_dis = self.local_polar_representation(xyz, neigh_idx)
        ...
        # f_pc_agg1 = self.att_pooling(f_concat, d_out, name + 'att_pooling_1', is_training)
        # f_pc_agg2 = self.att_pooling(f_concat, d_out, name + 'att_pooling_2', is_training)
        # f_pc_agg = f_pc_agg1 + f_pc_agg2
        f_pc_agg1 = helper_tf_util.conv2d(f_concat, d_out//2, [1, 16], name + 'mlp10', [1, 1], 'VALID', True, is_training)
        # f_neighbours = self.gather_neighbour(tf.squeeze(f_pc_agg, axis=2), neigh_idx)
        # f_concat = tf.concat([f_neighbours, f_xyz], axis=-1)
        # f_concat = f_neighbours
        f_pc_agg2 = helper_tf_util.conv2d(f_concat, d_out//2, [1, 16], name + 'mlp11', [1, 1], 'VALID', True,
                                          is_training)
        f_pc_agg = f_pc_agg1 + f_pc_agg2
        return f_pc_agg
    ...
    def local_polar_representation(self, xyz, neigh_idx):
        ...
        relative_info, relative_alpha, relative_beta, geometric_dis= self.relative_pos_transforming(xyz, neigh_idx, neighbor_xyz)
        angle_updated = tf.concat([relative_alpha, relative_beta], axis=-1)
        # Generate local spatial representation
        local_rep = tf.concat([angle_updated, relative_info], axis=-1)

        return local_rep,geometric_dis
    ...
    def relative_pos_transforming(self, xyz, neigh_idx, neighbor_xyz):
        ...
        relative_dis = tf.exp(-relative_dis)
        return relative_info, relative_alpha, relative_beta, relative_dis
    ...
    @staticmethod
    def att_pooling1(feature_set,f_dis, g_dis, d_out, name, is_training):
        batch_size = tf.shape(feature_set)[0]
        num_points = tf.shape(feature_set)[1]
        num_neigh = tf.shape(feature_set)[2]
        d = feature_set.get_shape()[3].value
        ...
        f_reshaped = tf.reshape(feature_set, shape=[-1, num_neigh, d])
        f_dis_reshaped = tf.reshape(f_dis, shape=[-1, num_neigh, d_dis])
        concat = tf.concat([f_dis_reshaped, f_reshaped], axis=-1)
        att_activation = tf.layers.dense(concat, d, activation=None, use_bias=False, name=name + 'fc')
        att_scores = tf.nn.softmax(att_activation, axis=1)
        f_agg = f_reshaped * att_scores
        f_agg = tf.reduce_sum(f_agg, axis=1)
        f_agg = tf.reshape(f_agg, [batch_size, num_points, 1, d])
